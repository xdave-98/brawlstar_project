...
class PlayerData(BaseModel):
    ...
    best_time_as_big_brawler: Optional[int] = Field(
        default=None,
        alias="bestTimeAsBigBrawler",
        description="Best time as Big Brawler",
    )
    ...
    # Brawlers
    brawlers: List[Brawler] = Field(
        default_factory=list, description="List of brawlers"
    )
...
class FlattenedPlayerData(BaseModel):
    ...
    trophies: int = Field(description="Current trophies", ge=0)
    highest_trophies: int = Field(
        alias="highestTrophies", description="Highest trophies achieved", ge=0
    )
    ...
    solo_victories: int = Field(
        alias="soloVictories", description="Solo victories", ge=0
    )
